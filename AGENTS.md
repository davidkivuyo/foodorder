...
# Remove the current default account profile image and add a "first letters of the current authenticated names approach" instead.

Requirements:

Remove the current hardcoded profile image which appear on every account screen of every user and add a custom stylish rounded border square with custom colour which is generated automatically for every user, inside the square there must be uppercase letters for the first letter of the name of the current logged in user. if the user has two names consider printing the combination of the first letters of each name. 

for example: if the user name is david, the uppercase letter printed will be D. But if the user name is david larabal, then the uppercase letter will be DL.
